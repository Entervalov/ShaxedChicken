...
public class DroneRenderer extends MobRenderer<AdvancedDroneEntity, ChickenModel<AdvancedDroneEntity>> {
    ...
    // Используем текстуру белого бетона как "белый пиксель" для фона текста
    ...
    public void render(@Nonnull AdvancedDroneEntity entity, float entityYaw, float partialTicks,
        ...
        // 1. ЭФФЕКТЫ (Пульсация для ядерных)
        if (type.isNuclear()) {
        ...
        // 3. ДЕБАГ ИНФОРМАЦИЯ И ЛИНИИ ПУТИ — только при включенном дебаге
    ...
    private void renderPathLine(AdvancedDroneEntity entity, MatrixStack ms, IRenderTypeBuffer buffer, float partialTicks) {
        ...
        // Получаем буфер для линий. RenderType.lines() всегда рисует поверх блоков (depthTest=false обычно)
        // или используем lines() который стандартный.
        IVertexBuilder lineBuilder = buffer.getBuffer(RenderType.lines());

        ms.pushPose();

        // Вычисляем координаты для отрисовки линии.
        // Мы находимся в локальных координатах сущности (после translate в методе render основного рендерера EntityRendererManager)
        // Но так как мы сделали popPose выше, мы сейчас в координатах сущности, но без поворотов модели.

        // Интерполяция позиции дрона для плавности
        ...
        // Вектор от центра дрона до центра целевого блока
        ...
        // Цвет линии зависит от состояния AI
        float r = 0, g = 1, b = 0; // Зеленый (Patrol)
        switch (entity.getAIState()) {
            case ATTACK: r=1; g=0; b=0; break; // Красный
            case DIVE_BOMB: r=1; g=0; b=0; break;
            case CHASE: r=1; g=1; b=0; break; // Желтый
            case SEARCH: r=0; g=1; b=1; break; // Голубой
            case RETREAT: r=1; g=0; b=1; break; // Фиолетовый
            case EVADE: r=1; g=0.5f; b=0; break; // Оранжевый
        }

        // Рисуем линию от (0, 0.5, 0) - центр дрона, до (dx, dy, dz)
    ...
    private void renderDebugInfo(AdvancedDroneEntity entity, MatrixStack ms, IRenderTypeBuffer buffer) {
        ...
        // Поднимаем текст над головой (учитываем реальную высоту хитбокса)
        ...
        // Отрицательный scale переворачивает текст правильно
        ...
        // Используем RenderType.text(TEXTURE) с белой текстурой, чтобы можно было наложить цвет (tint)
        ...
        float a = 0.65F; // Прозрачность фона (0.0 - 1.0)

        // Рисуем квадрат (2 треугольника = 4 вершины квада)
        // Важно: UV координаты 0,0 -> 1,1 чтобы взять всю текстуру
        ...
        for (String line : lines) {
            ...
            // transparent=false, seeThrough=false (чтобы текст не просвечивал сквозь стены)
