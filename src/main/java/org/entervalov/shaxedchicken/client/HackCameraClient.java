...
public class HackCameraClient {
    ...
    // Трекинг камеры через дельты мыши
    ...
    // === ПЕРЕКЛЮЧЕНИЕ КАМЕРЫ (Вызывается из пакета) ===
    ...
    // === ПЕРЕХВАТ ESC (выход из дрона вместо открытия меню паузы) ===
    @SubscribeEvent
    ...
    private static void disconnectDrone() {
        ...
        if (cam instanceof AdvancedDroneEntity) {
            ...
            // Отправляем exit-пакет на сервер
    ...
    public static void onRenderOverlay(RenderGameOverlayEvent.Post event) {
        ...
        if (isActive && mc.getCameraEntity() instanceof AdvancedDroneEntity) {
            ...
            // 4. Инфо-текст (нормальный масштаб)
            int infoY = 24;
    ...
    private static void renderJammedNoise(MatrixStack ms, int w, int h, AdvancedDroneEntity drone) {
        // Красные/белые полосы помех — интенсивность зависит от расстояния до глушилки
        double dist = drone.getDistanceToJammer();
        float intensity = (float) Math.max(0.1, 1.0 - (dist / 30.0)); // Ближе = сильнее
    ...
    public static void onClientTick(TickEvent.ClientTickEvent event) {
        ...
        if (isActive && mc.getCameraEntity() instanceof AdvancedDroneEntity && mc.player != null) {
            ...
            // Вычисляем дельту мыши через изменение ротации игрока
            ...
            // Применяем к дрону на клиенте для отображения камеры
            drone.yRot = droneYaw;
            ...
            // Считываем инпут
            boolean attack = mc.mouseHandler.isLeftPressed(); // ЛКМ = Взрыв
            boolean sneak = mc.options.keyShift.isDown(); // Shift = Спуск
            boolean jump = mc.options.keyJump.isDown(); // Пробел = Подъём
            ...
            // Отправляем пакет (exit = false, выход только через Esc)
            ModNetwork.CHANNEL.sendToServer(new DroneInputPacket(
                    drone.getId(),
                    attack,
                    false,
            ...
            // Блокируем стандартную обработку кликов
