...
public class ModCommonEvents {
    ...
    private static final int SPAWN_INTERVAL = 1200; // Раз в минуту
    private static final int MAX_DRONES_NEAR_PLAYER = 2;
    private static final double CHECK_RANGE = 100.0; // Небольшой радиус!
    ...
    public static void onServerTick(TickEvent.WorldTickEvent event) {
        ...
        // Выбираем ОДНОГО случайного игрока
        ServerPlayerEntity player = players.get(random.nextInt(players.size()));

        // 15% шанс спавна
        if (random.nextFloat() > 0.15f) return;

        // КЛЮЧЕВОЕ: Проверяем дронов только рядом с игроком (НЕ весь мир!)
