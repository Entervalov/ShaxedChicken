...
public class JammerTileEntity extends TileEntity implements ITickableTileEntity, IInventory, INamedContainerProvider {
    ...
    public void tick() {
        ...
        if (mode == JammerMode.ALL || mode == JammerMode.JAMMER) {
            // Глушим каждые 5 тиков, чтобы эффект не спадал
    ...
    private void updateTargets() {
        boolean changed = targetEntityIds.removeIf(id -> {
            ...
            // Убираем взломанных дронов из целей лазера
        ...
        if (targetEntityIds.size() < getMaxTargets()) {
            ...
            for (AdvancedDroneEntity drone : drones) {
                ...
                if (drone.isHacked()) continue; // Лазер не стреляет по взломанным дронам
    ...
    private void jamNearbyDrones() {
        ...
        for (AdvancedDroneEntity drone : drones) {
            // Передаем координаты этого TileEntity (this.worldPosition)
            drone.setJammed(this.worldPosition, getJammerDurationTicks()); // модульная длительность

            // Сбрасываем атаку, чтобы дрон сразу отреагировал
