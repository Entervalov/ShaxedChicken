...
public class AdvancedDroneEntity extends FlyingEntity implements IMob {
        ...
        if (!this.level.isClientSide) {
            ...
            if (this.jammedTicks > 0) {
                ...
                if (jammerPos != null) {
                    ...
                    if (dist < 10.0) {
                        // Если дрон взломан — разрываем связь перед крашем
                        ...
                        // Для взломанных дронов: помехи, но не полная потеря контроля
                        if (!isHacked()) {
                            handleInterference();
                        }
                        // Взломанный дрон: РЭБ мешает, но игрок всё ещё управляет (с помехами на HUD)
                    }
                } else {
            ...
            // Если взломан - обычный AI отключен, работает ручное управление или handleHackedBehavior
            if (isHacked()) {
    ...
    // === МЕТОДЫ ПОВЕДЕНИЯ (КОТОРЫХ НЕ БЫЛО) ===
    ...
    private boolean handleHackedBehavior() {
        ...
        return false; // Иначе ничего не делаем, ждем инпута игрока
    }
    ...
    private void tickHacking() {
        ...
        // Частицы от игрока к дрону (enchant sparkles)
        if (hackingTicks % 4 == 0) {
    ...
    private void completeHacking(ServerPlayerEntity player) {
        ...
        // Звук и эффект успешного взлома
    ...
    public void handleInput(boolean attack, boolean exit, float yRot, float xRot, float forward, float strafe, boolean jump, boolean sneak) {
        ...
        // Вертикальное движение: Пробел — вверх, Shift — вниз
        if (jump) {
            motion = motion.add(0, 0.4, 0);
        } else if (sneak) {
            motion = motion.add(0, -0.3, 0);
        } else {
            // Зависание на месте (медленный спуск)
    ...
    private void sendCameraPacket(ServerPlayerEntity player, boolean enable) {
        // ИСПРАВЛЕНИЕ: Используем PacketDistributor напрямую
    ...
    private void handleClientEffects() {
        // Частицы в зависимости от состояния
        if (isJammed()) {
            // Дым и искры при глушении
