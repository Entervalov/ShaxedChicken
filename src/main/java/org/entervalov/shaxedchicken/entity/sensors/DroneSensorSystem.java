...
public class DroneSensorSystem {
    ...
    public boolean canSeeTarget(LivingEntity target) {
        // 1. ПРОВЕРКА ГЛУШЕНИЯ (Вероятностная слепота)
        if (drone.isJammed()) {
            double dist = drone.getDistanceToJammer();

            // Если мы в мертвой зоне (<10) - мы слепы на 100%
            if (dist < CRITICAL_RANGE) return false;

            // Расчет качества сигнала (0.0 = плохо, 1.0 = отлично)
            // Пример: Дистанция 20. (20-10)/(30-10) = 10/20 = 0.5 (50% шанс)
            double signalQuality = (dist - CRITICAL_RANGE) / (MAX_JAMMER_RANGE - CRITICAL_RANGE);

            // Если случайное число (0.0-1.0) больше качества сигнала - цель не видим
    ...
    public void updateRadar() {
        ...
        // 1. ПРОВЕРКА ГЛУШЕНИЯ (Сбои радара)
        if (drone.isJammed()) {
            double dist = drone.getDistanceToJammer();
            // В мертвой зоне радар отключается полностью
            if (dist < CRITICAL_RANGE) return;

            // Шанс обновления радара (чем ближе, тем реже обновляется)
            // Если сигнал 0.2 (очень плохой), то шанс обновления всего 20%
            double signalQuality = (dist - CRITICAL_RANGE) / (MAX_JAMMER_RANGE - CRITICAL_RANGE);
            if (drone.getRandom().nextFloat() > signalQuality) {
                return; // Радар "помеховал" и не обновился в этот тик
            }
        }
    ...
    public LivingEntity selectBestTarget() {
        if (drone.isJammed()) return null; // НЕ МОЖЕТ ВЫБРАТЬ ЦЕЛЬ
