...
public class ModConfiguredStructures {
    ...
    public static void registerConfiguredStructures() {
        // 1. Register separation settings into the DEFAULTS map
        ...
        // 2. Register configured structure features
    ...
    private static <F extends Structure<?>> void registerStructureSeparation(
        ...
        // Register into Structure.STRUCTURES_REGISTRY — key must be just the path, not mod_id:path
        ...
        // Patch DimensionStructuresSettings.DEFAULTS using ObfuscationReflectionHelper-safe approach
    ...
    private static void patchDefaults(Structure<?> structure, StructureSeparationSettings settings) {
        // DimensionStructuresSettings.DEFAULTS is an ImmutableMap, field_236191_b_ in SRG names.
        // We need to try both the deobfuscated and SRG name to support dev and prod environments.
        try {
            Field defaultsField = null;

            // Try SRG name first (production environment)
            for (Field f : DimensionStructuresSettings.class.getDeclaredFields()) {
                if (f.getType() == Map.class || f.getType().getName().contains("ImmutableMap")) {
                    // Check if this is a static field with the right type signature
                    if (java.lang.reflect.Modifier.isStatic(f.getModifiers())
                        ...
                        if (val instanceof Map) {
                            Map<?, ?> map = (Map<?, ?>) val;
                            // Verify this is the DEFAULTS map by checking if it contains vanilla structures
            ...
            // Prefer Unsafe: reliably updates static final across Java versions
            ...
            // Fallback: try to clear final modifier and set via reflection (best-effort)
            try {
