...
/**
 * Заброшенная лаборатория — подземная структура.
 * Две комнаты: лабораторная и склад БПЛА.
 * Редкий лут, спавнер дронов, лестница на поверхность.
 */
public class AbandonedLabPiece extends StructurePiece {
    ...
    public AbandonedLabPiece(BlockPos origin) {
        ...
        this.origin = origin;
        // Generous bounding box that covers from the structure to the surface (for ladder shaft)
    ...
    public boolean postProcess(@Nonnull ISeedReader world,
        ...
        // === ОСНОВНАЯ ОБОЛОЧКА: две комнаты с перегородкой ===
        for (int dx = 0; dx < WIDTH; dx++) {
            for (int dy = 0; dy < HEIGHT; dy++) {
                for (int dz = 0; dz < DEPTH; dz++) {
                    ...
                    if (isFloor) {
                        // Пол: чередование полированного андезита и каменных кирпичей
                        if ((dx + dz) % 3 == 0) {
                        ...
                        // Перегородка с двумя дверными проёмами
        ...
        // === ЛАБОРАТОРНАЯ КОМНАТА (левая сторона, dx 1-5) ===

        // Лабораторные столы (слэбы)
        ...
        // Паутины (атмосфера заброшенности)
        int[][] cobwebPositions = {
        ...
        // Разбитые стеклянные панели (осколки экспериментов)
        ...
        // === СКЛАД БПЛА (правая сторона, dx 7-11) ===

        // Основной сундук с редким лутом
        ...
        // Заготовки дронов (стойки для брони — "стенды дронов")
        ...
        // === СПАВНЕР ДРОНОВ в центре лаборатории ===
        ...
        // Красные факелы — атмосфера тревоги
        ...
        // Маркер на поверхности — чтобы было видно вход
